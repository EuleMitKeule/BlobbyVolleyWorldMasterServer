...
using System;
using Task = BeardedManStudios.Threading.Task;
...
namespace MasterServer
{
	public class MasterServer
	{
		...
		private void Register(NetworkingPlayer player, JSONNode data)
		{
			...
			if (comment == "none")
			{
				address = ((IPEndPoint)player.TcpClientHandle.Client.RemoteEndPoint).Address.ToString();
			}
			else
			{
			...
			hosts.Add(host);
			Log(string.Format($"registered new server\n" +
							  $"gameID: {gameId}\n" +
							  $"type: {gameType}\n" +
							  $"name: {name}\n" +
							  $"address: {address}\n" +
